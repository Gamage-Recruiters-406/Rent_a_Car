...
import { FileText, Car, User, Inbox } from "lucide-react";
...
const BookingTable = ({ data = [], loading = false, onViewAction }) => {
  ...
  if (loading) {
    return <div className="p-10 text-center font-['Nunito'] text-gray-500 italic">Loading bookings...</div>;
  }
        ...
        <table className="w-full text-center border-collapse">
          ...
          <tbody className="text-gray-700">
            {data.length > 0 ? (
              ...
              <tr>
                <td colSpan={tableHeaders.length} className="px-4 py-20 text-gray-400">
                  <div className="flex flex-col items-center justify-center">
                    <Inbox size={40} className="mb-2 opacity-20" />
                    <p className="italic">No bookings available at the moment.</p>
                  </div>
                </td>
              </tr>
            )}
