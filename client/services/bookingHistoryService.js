...
export const extractOwnerInfo = (ownerId, includeEmail = false) => {
  ...
  if (typeof ownerId === 'object' && ownerId !== null) {
    return {
      name:
        `${ownerId.first_name || ''} ${ownerId.last_name || ''}`.trim() ||
        'Owner',
  ...
  try {
    ...
    if (response.ok && data?.success && data?.user) {
      ...
      if (response.status === 401) {
        ...
        await AsyncStorage.multiRemove(['userId', 'userRole', 'userStatus']);
      }
  ...
  try {
    ...
    if (error.response) {
      return {
        ...
        data: null,
        message:
          error.response.data?.message || 'Error loading booking details',
...
export const enrichBookingForHistory = async (
  booking,
  API_BASE_URL,
  API_VERSION,
) => {
    ...
    if (booking.vehicleId) {
      ...
      if (typeof booking.vehicleId === 'object' && booking.vehicleId !== null) {
        vehicleDetails = {
          ...
          transmission: booking.vehicleId.transmission,
...
export const enrichBookingData = async (
  bookingData,
  API_BASE_URL,
  API_VERSION,
) => {
      ...
      try {
        ...
        if (
          ...
          vehicleDetails.title =
            bookingData.vehicleId.title || 'Unknown Vehicle';
          vehicleDetails.model = bookingData.vehicleId.model || 'N/A';
          vehicleDetails.numberPlate =
            bookingData.vehicleId.numberPlate || 'N/A';
...
export const fetchAndEnrichCustomerBookings = async (
  userId,
  API_BASE_URL,
  API_VERSION,
) => {
  try {
    const bookingsResponse = await fetchCustomerBookings(
      userId,
      API_BASE_URL,
      API_VERSION,
    );
    ...
    const enrichedBookings = await Promise.all(
      bookingsResponse.data.map(async (booking) => {
        try {
          const enriched = await enrichBookingForHistory(
            booking,
            API_BASE_URL,
            API_VERSION,
          );
...
export const getEnrichedBooking = async (
  bookingId,
  API_BASE_URL,
  API_VERSION,
) => {
  try {
    const bookingResponse = await fetchBookingDetails(
      bookingId,
      API_BASE_URL,
      API_VERSION,
    );
    ...
    if (!bookingResponse.success) {
      throw new Error(
        bookingResponse.message || 'Failed to load booking details',
      );
    }

    const enrichedBooking = await enrichBookingData(
