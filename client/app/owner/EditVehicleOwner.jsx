...
// ────────────────────────────────────────────
const DropdownPicker = ({ label, required, options, value, onSelect, placeholder, disabled }) => {
    const [open, setOpen] = useState(false);
    return (
        <View style={{ marginBottom: 16 }}>
            <Text style={{ color: "#0D3778", fontWeight: "600", fontSize: 14, marginBottom: 6 }}>
            ...
            >
                ...
                <Ionicons name="chevron-down" size={18} color={disabled ? "#9CA3AF" : "#0D3778"} />
            </TouchableOpacity>
            ...
            <Modal visible={open} transparent animationType="slide">
                ...
                >
                    <View style={{ backgroundColor: "white", borderTopLeftRadius: 16, borderTopRightRadius: 16, maxHeight: "60%" }}>
                        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: "#E5E7EB" }}>
                            <Text style={{ fontSize: 18, fontWeight: "700", color: "#0D3778" }}>Select {label}</Text>
                        ...
                        <ScrollView style={{ paddingHorizontal: 8, paddingVertical: 8 }}>
                            {options.map((opt) => (
                                ...
                                >
                                    <Text style={{ fontSize: 16, color: value === opt ? "#1D4ED8" : "#374151", fontWeight: value === opt ? "600" : "400" }}>
                                        {opt}
                                    </Text>
                                </TouchableOpacity>
                            ))}
...
export default function EditVehicleOwnerScreen() {
    ...
    const fetchVehicleData = async () => {
        try {
            ...
            if (vehicle.location?.geo?.coordinates) {
                ...
                setSelectedLocation({ lat, lng, address: vehicle.location.address || "" });
            }
        } catch (error) {
    ...
    const handleSubmit = () => {
        const requiredFields = [
            "year", "fuelType", "description", "km", "seats",
            "pricePerDay", "pricePerKm", "transmission", "address",
        ];
        ...
        if (replacePhotos) {
            if (newPhotos.length === 0) {
                Alert.alert("Validation", "Please upload new photos to replace existing ones.");
                return;
            }
        } else {
            if (existingPhotos.length === 0 && newPhotos.length === 0) {
                Alert.alert("Validation", "Please keep at least 1 photo or upload new photos.");
                return;
            }
        }
    ...
    if (fetchingData) {
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: "#F3F4F6", justifyContent: "center", alignItems: "center" }}>
                ...
                <Text style={{ marginTop: 16, color: "#6B7280", fontSize: 14 }}>Loading vehicle data...</Text>
            </SafeAreaView>
        );
    }
                ...
                >
                    ...
                    >
                        ...
                        <View style={{ marginBottom: 20 }}>
                            <Text style={{ fontSize: 24, fontWeight: "700", color: "#0D3778", marginBottom: 4 }}>
                        ...
                        {/* ─── Read-only fields ─── */}
                        <ReadOnlyField label="Vehicle Title" value={formData.title} placeholder="e.g., Toyota Camry 2020" />

                        <View style={{ flexDirection: "row", gap: 12 }}>
                            <View style={{ flex: 1 }}>
                                <ReadOnlyField label="Vehicle Model" value={formData.model} placeholder="e.g., Toyota" />
                            </View>
                            <View style={{ flex: 1 }}>
                                <ReadOnlyField label="Vehicle Type" value={formData.vehicleType} placeholder="—" />
                            </View>
                        </View>
                        ...
                        <View style={{ flexDirection: "row", gap: 12, marginBottom: 16 }}>
                            <View style={{ flex: 2 }}>
                                <ReadOnlyField label="Number Plate" value={formData.numberPlate} placeholder="WP/AB 1234" />
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={{ color: "#0D3778", fontWeight: "600", fontSize: 14, marginBottom: 6 }}>
                        ...
                        {/* ─── Rental Amount ─── */}
                        <View style={{ marginBottom: 16 }}>
                            <Text style={{ color: "#0D3778", fontWeight: "600", fontSize: 14, marginBottom: 8 }}>
                                Rental Amount <Text style={{ color: "#EF4444" }}>*</Text>
                            </Text>
                            <View style={{ flexDirection: "row", gap: 12 }}>
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 12, color: "#6B7280", marginBottom: 4 }}>Daily Rental Rate</Text>
                                    <TextInput
                                        style={{
                                            borderWidth: 2,
                                            borderColor: "#0D3778",
                                ...
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 12, color: "#6B7280", marginBottom: 4 }}>Per Kilometer Charge</Text>
                                    <TextInput
                                        style={{
                                            borderWidth: 2,
                                            borderColor: "#0D3778",
                        ...
                        <View style={{ marginBottom: 16 }}>
                            ...
                            {existingPhotos.length > 0 && (
                                <View style={{ marginBottom: 12 }}>
                                    <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 8 }}>
                                        ...
                                        <Text style={{ color: "#0D3778", fontWeight: "600", fontSize: 13, marginLeft: 6 }}>
                            ...
                            >
                                <Text style={{ color: "#374151", fontSize: 13, flex: 1, marginRight: 8 }}>
                            ...
                            {replacePhotos && (
                                ...
                                >
                                    ...
                                    <Text style={{ color: "#92400E", fontSize: 12, flex: 1 }}>
                                        <Text style={{ fontWeight: "700" }}>Replace Mode Active: </Text>
                                        All current photos will be removed and replaced with your new uploads when you save.
                                    </Text>
                                </View>
                            )}
                            ...
                            <TouchableOpacity
                                ...
                                activeOpacity={0.7}
                            >
                                <Ionicons name="cloud-upload-outline" size={48} color="#0D3778" />
                                <Text style={{ color: "#374151", fontWeight: "500", marginTop: 8 }}>
                            ...
                            {newPhotos.length > 0 && (
                                <View style={{ marginTop: 12 }}>
                                    <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 8 }}>
                                        <Ionicons name="images-outline" size={18} color="#0D3778" />
                                        <Text style={{ color: "#0D3778", fontWeight: "600", fontSize: 13, marginLeft: 6 }}>
                        ...
                        <View style={{ flexDirection: "row", gap: 12, marginTop: 8 }}>
                            <TouchableOpacity
                                ...
                                activeOpacity={0.7}
                            >
                                <Ionicons name="arrow-back-outline" size={20} color="#0D3778" />
                                <Text style={{ color: "#0D3778", fontWeight: "600", marginLeft: 8 }}>Cancel</Text>
                            </TouchableOpacity>
